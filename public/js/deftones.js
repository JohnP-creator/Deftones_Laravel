...
document.addEventListener('DOMContentLoaded', function() {
    ...
    navLinks.addEventListener('click', function(e) {
        if (e.target.tagName === 'A') {
            // Añadir animación de cierre incluso para enlaces externos
            closeMobileMenu();
            
            // Si es un enlace a otra página, añadir un pequeño delay
            // para que se vea la animación antes de navegar
            const href = e.target.getAttribute('href');
            if (href && !href.startsWith('#') && !href.startsWith('index.html#')) {
                e.preventDefault();
                setTimeout(() => {
                    window.location.href = href;
                }, 300); // Delay de 300ms para ver la animación
            }
        }
    });
